 ...
 * @author kimidori
 */
public class FeatureGeneratorCB_20180710 implements FeatureGenerator{
	...
	public ArrayList<String> getResourceFilePath(){
		String treefile="resources/cbpred/tree_simple.0.dat";
		String scorefile="resources/cbpred/scores.0.dat";
	...
	public static ArrayList<String> getHeader(){
		...
		for(String r:aaname){
			for(int ii = 0;ii < 3;ii++){
				headers.add(r+"_JCB_dist"+String.valueOf(ii));
				headers.add(r+"_JCB_distdiff"+String.valueOf(ii));
			}
		}
		return headers;
	}
	...
	public ArrayList<Double> generateFeatures(PDBAtom tcbb){
		...
		*/
		...
		//ret.add((double)cacount2[0]-cacount[0]);
		//ret.add((double)cacount2[1]-cacount[1]);
		//ret.add((double)cacount2[2]-cacount[2]);
		...
		//jCB について
		HashMap<String,ArrayList<AtomDistance>> jcbcount = new HashMap<>();
		for(String s:aaname){
			jcbcount.put(s,new ArrayList<AtomDistance>());
		}
		for(PDBResidue r:jCB.keySet()){
			if(r == target){
				continue;
			}
			PDBAtom a = jCB.get(r);
			double tdis = a.distance(tcb1);
			if(a.distance(tcb1) > 12.0){
				continue;
			}
			double tdis2 = a.distance(tcb2);
			String rn = r.getName();
			if(!validResidue.contains(rn)){
				System.err.println(rn+" is changed to MET.");
				rn = "MET";//特に意味はないがメチオニンにする
			}
			jcbcount.get(rn).add(new AtomDistance(a,tdis,tdis2-tdis));
			
		}
		for(String s:aaname){
			ArrayList<AtomDistance> dlis = jcbcount.get(s);
			Collections.sort(dlis,new AtomDistanceComparator());
			for(int jj = 0;jj < 3;jj++){
				if(dlis.size() > jj){
					ret.add(dlis.get(jj).distance);
	...
	public static ArrayList<String> countAtoms_PDB(String treefile
		...
		for(File f:lis){
			...
			FeatureGeneratorCB_20180710 gen = new FeatureGeneratorCB_20180710();
...
//trueatom_predictedas ALA_CB@5:	ALA_CB	342	GLY_PCB	197	SER_CB	108	LEU_CB	63	PHE_CB	48	VAL_CB	44	THR_CB	43	PRO_CB	40	TYR_CB	35	CYS_CB	28	ILE_CB	24	MET_CB	17	HIS_CB	17	ASN_CB	16	ASP_CB	16	LYS_CB	15	GLN_CB	12	TRP_CB	12	GLU_CB	10	ARG_CB	3
	...
	public static void makeScoreTable(String[] args){
		...
		//"C:\\dummy\\vbox_share\\bioo\\database\\for_energyfunction\\14_cb\\tree_simple.0.dat_0"
		//,"C:\\dummy\\vbox_share\\bioo\\database\\for_energyfunction\\14_cb\\input.0.train.dat");
		//for(String s:res){
		//	System.out.println(s);
		//}
		//printStrings("C:\\dummy\\vbox_share\\bioo\\database\\for_energyfunction\\14_cb\\scores.0.dat",res);
		
		
		String sampledirname = "C:\\dummy\\vbox_share\\bioo\\database\\for_energyfunction\\14_cb\\pdb_train";
		File dir = new File(sampledirname);
		File[] lis = dir.listFiles();
		
		String testdirname = "C:\\dummy\\vbox_share\\bioo\\database\\for_energyfunction\\14_cb\\pdb_test";
		File testdir = new File(testdirname);
		File[] lis_test= testdir.listFiles();
		
		ArrayList<String> dres = countAtoms_PDB(
		//"C:\\dummy\\vbox_share\\bioo\\database\\for_energyfunction\\14_cb\\tree_simple.0.dat_0"
		"C:\\dummy\\vbox_share\\bioo\\database\\for_energyfunction\\14_cb\\reftree_noresinfo.0.20.dat"
		,lis,lis_test);
		for(String s:dres){
		//	System.out.println(s);
		}
		printStrings("C:\\dummy\\vbox_share\\bioo\\database\\for_energyfunction\\14_cb\\scores.0.dat_b",dres);
		
	}
	public static void makeInputTable(String[] args){
		String outfilename = "C:\\dummy\\vbox_share\\bioo\\database\\for_energyfunction\\table_cb.dat";
		...
		try{
			...
			ArrayList<String> header = FeatureGeneratorCB_20180710.getHeader();
			pw.write("name\t");
			for(String h:header){
				pw.write(h+"\t");
			}
	...
	public static void createScoringLine(String pdbdir, String treefile,String scorefile,String outfile){
		FeatureGeneratorCB_20180710 gcb = new FeatureGeneratorCB_20180710();
	...
	public static void main(String[] args){
		...
		for(int ii = 0;ii < 10;ii++){
			String treedir = "C:\\dummy\\vbox_share\\bioo\\database\\for_energyfunction\\14_cb\\fintree\\";
			String basedir = "C:\\dummy\\vbox_share\\bioo\\database\\for_energyfunction\\14_cb\\";
