...
public class RandomDocker {
	...
	public static void main__(String[] args){
		...
		for(String cname:p.chains.keySet()){
			...
			HashMap<PDBResidue,Integer> pi = RandomDocker.getSurfaceResidues(c.residues,false);
	...
	public static void rotates(ArrayList<Point3D> al,Point3D center_,double rx,double ry,double rz){
		double cx = center_.x;
		double cy = center_.y;
		double cz = center_.z;
		for(Point3D pp:al){
	...
	,double restrictdist){
		...
		HashMap<PDBResidue,Integer> pi = RandomDocker.getSurfaceResidues(targetres,false);
		HashMap<PDBResidue,Integer> pi2 = RandomDocker.getSurfaceResidues(templateres,false);
	...
	public static void main(String[] args){
		RandomDocker sd = new RandomDocker();
		PDBData p  = PDBData.loadPDBFile("C:\\dummy\\vbox_share\\casp13\\queries\\T0966\\re_model1_model2_g.pdb.pr.pdb");
		ArrayList<PDBResidue> templateres = new ArrayList<>();
		for(String cname:p.chains.keySet()){
			PDBChain c = p.chains.get(cname);
		...
		PDBData p2  = PDBData.loadPDBFile("C:\\dummy\\vbox_share\\casp13\\queries\\T0966\\re_model1_model2_g.pdb.pr.pdb");
		ArrayList<PDBResidue> targetres = new ArrayList<>();
		...
		for(int ii = 0;ii < candidates.size();ii++){
			...
			writeToFile(makeAtomLines(targetres,1,1,"B"),
					"C:\\dummy\\vbox_share\\casp13\\queries\\T0966\\dockz_model1."+dcount+".pdb");
						dcount++;
						break outer;
					}
				}
